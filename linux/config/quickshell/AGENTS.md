...
- On Quickshell 0.3.1/X11, `PopupWindow.grabFocus` alone does not reliably implement keyboard/pointer grabs. `ConnectivityCenter.qml` uses a non-grabbing hover preview and `Controls.Popup.Window` for the pinned view, sharing one content item. Preserve this distinction unless a replacement is live-verified. During pin promotion, keep a snapshot in the preview above the destination until the destination's first `frameSwapped`, matching the DateTimeCenter pattern to avoid a single-frame blink when a control is clicked in the hover-opened panel.
